...
export const signup = async (req, res) => {
  try {
    ...
    res.json({ status: 201, message: "User Created Successfully", success: true, data: newUser })
  }
  catch (err) {
...
export const login = async (req, res) => {
  try {
    ...
    if (!email || !password) {
      return res.status(404).json({ success: false, message: "add your credentials to proceed" })
    }
    ...
    const token = jwt.sign({ userId: findUser._id, email: findUser.email, password: findUser.password }, SECRET_KEY, { expiresIn: "1h" });
    res.status(200).json({ success: true, message: "user login successfully ", data: findUser, token })
  } catch (err) {
    console.log(err)
  }
};
...
export const deleteSingleUser = async (req, res) => {
  try {
    ...
    return res.status(500).json({ message: "internal server error in deleteSingleuser" })
  }
}
