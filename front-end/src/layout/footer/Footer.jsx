...
const Footer = () => {
  return (
    <footer className="bg-[#f5f4ee] h-min-[100vh] w-full overflow-y-auto py-[90px]">
            ...
            {/* footer header  */}
             <div className="sm:flex justify-between items-center w-full mx-auto">
                ...
                <div className="flex justify-center items-center sm:mt-0 mt-8">
                    <input type="text" className="h-[50px] max-w-[400px] min-w-[300px] border-0 outline-none bg-white text-gray-600 text-[14px] font-[300] px-[15px]"/>
                ...
                </div>
                {
                    footerData.map((item) => (
                        <div className="flex flex-col items-start gap-4" key={item.id}>
                            ...
                            {
                                item.links.map((link) => (
                                    <Link to={link.link} className="text-[14px] text-gray-500 font-[400]" key={link.id}>{link.name}</Link>
                                ))
                            }
                        </div>
                    ))
