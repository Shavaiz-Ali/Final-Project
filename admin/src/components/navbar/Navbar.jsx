...
const Navbar = () => {
    const [sideBar, setSideBar] = useState(window.innerWidth > 996);

    useEffect(() => {
        const responsiveNavbar = () => {
            setSideBar(window.innerWidth > 996);
        };
    ...
    return (
        <>
            ...
            >
                <span className="sr-only">{sideBar ? "Close sidebar" : "Open sidebar"}</span>
                ...
                <div className="h-full px-3 py-4 overflow-y-auto bg-gray-50 dark:bg-gray-800">
                    <ul className="space-y-2 font-medium">
                        {sideBarData.map((item) => (
                            <Link to={item.link} className="flex items-center p-2 text-gray-900 rounded-lg dark:text-white hover:bg-gray-100 dark:hover:bg-gray-700 group" key={item.id} onClick={
                                () => setSideBar(false)
                            }>
                                {item.icon}
